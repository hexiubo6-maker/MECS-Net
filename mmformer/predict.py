...
def save_array_as_nifti_gz(array, output_file_path):
    ...
    if not isinstance(array, np.ndarray) or len(array.shape) != 3:
        raise ValueError("输入必须是一个三维numpy数组")
        ...
        for h in h_idx_list:
            for w in w_idx_list:
                for z in z_idx_list:
                    ...
                    pred_part = pred_part[:,0:4,:,:,:]
                    pred[:, :, h:h+patch_size, w:w+patch_size, z:z+patch_size] += pred_part
        pred = pred / weight
        b = time.time()
        # crop to actual input depth Z to match target, not fixed T
        pred = pred[:, :, :H, :W, :Z]
        pred = torch.argmax(pred, dim=1)

        name = names[0][-3:]
        save_array_as_nifti_gz(pred, "C:\\Users\\xiubo\\PycharmProjects\\hfn\\t1\\"+str(name)+".nii.gz")
