...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.edge_encode_conv = nn.Conv3d(basic_dims*16, transformer_basic_dims, kernel_size=1, stride=1, padding=0)
        ...
        self.edge_pos = nn.Parameter(torch.zeros(1, patch_size**3, transformer_basic_dims))
        ...
        self.edge_transformer = Transformer(embedding_dim=transformer_basic_dims, depth=depth, heads=num_heads, mlp_dim=mlp_dim)
        ...
        self.decoder_fuse = Decoder_fuse(num_cls=5)
    ...
    def forward(self, x, mask):
        ...
        edge_x1, edge_x2, edge_x3, edge_x4, edge_x5 = self.edge_encoder(x[:, 4:5, :, :, :])
        ...
        edge_token_x5 = self.edge_encode_conv(edge_x5).permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), -1, transformer_basic_dims)
        ...
        edge_intra_token_x5 = self.edge_transformer(edge_token_x5, self.edge_pos)
        ...
        # flair_x1(1, 8, 128, 128, 128) flair_x2(1, 16, 64, 64, 64) flair_x3(1, 32, 32, 32, 32) flair_x4(1, 64, 16, 16, 16) flair_x5(1, 128, 8, 8, 8)
        # flair_pred(1, 4, 128, 128, 128)
        if self.is_training:
        ...
        x1 = self.masker(torch.stack((flair_x1, t1ce_x1, t1_x1, t2_x1, edge_x1), dim=1), mask) #Bx4xCxHWZ
        x2 = self.masker(torch.stack((flair_x2, t1ce_x2, t1_x2, t2_x2, edge_x2), dim=1), mask)
        x3 = self.masker(torch.stack((flair_x3, t1ce_x3, t1_x3, t2_x3, edge_x3), dim=1), mask)
        x4 = self.masker(torch.stack((flair_x4, t1ce_x4, t1_x4, t2_x4, edge_x4), dim=1), mask)
        x5_intra = self.masker(torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5, edge_intra_x5), dim=1), mask)
        # x5_intra = self.masker(torch.stack((flair_intra_x5, t1ce_intra_x5, t1_intra_x5, t2_intra_x5, edge_intra_x5), dim=1), mask)
        ...
        multimodal_pos = torch.cat((self.flair_pos, self.t1ce_pos, self.t1_pos, self.t2_pos, self.edge_pos), dim=1)
